...
function imageDataToPowerArray (data) {
    ...
    for (var ix = 0; ix < length; ix++) {
        if (data.data[ix * 4 + 3] != 255) {
            ...
            arr[ix] = Math.sqrt(
                Math.pow(data.data[ix * 4 + 0], 2)
                + Math.pow(data.data[ix * 4 + 1], 2)
                + Math.pow(data.data[ix * 4 + 2], 2)
            ) / 255 / SQRT3;
        }
    }
        ...
        onFileChange: function (e) {
            ...
            loadFile(e.target.files[0], function (blobUrl) {
                generateImageData(blobUrl, function (imageData) {
                    ...
                    vm.source.powerArray = imageDataToPowerArray(imageData);
                    vm.refreshSlice1();
                    vm.refreshSlice2();
